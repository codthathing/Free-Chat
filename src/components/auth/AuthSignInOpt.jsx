  ...
  return (
    <div className="flex flex-col gap-y-3 md:gap-y-4 lg:gap-y-3">
      {array.map(({ id, icon, text, navigate }) => {
        return (
          <main key={id} onClick={() => navigate && navigateTo(navigate)} className="flex gap-x-1 md:gap-x-5 lg:gap-x-4 justify-center items-center cursor-pointer border-gray-light border md:border-2 rounded-md md:rounded-xl lg:rounded-lg py-2 md:py-3.5">
